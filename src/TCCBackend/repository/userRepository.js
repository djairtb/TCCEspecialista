...
let userRegister = async (login, passwordHash,email,nome) => {
    try {
        await db.pool.query('INSERT INTO public.users u (login, passwordHash, email, nome) VALUES ($1,%2,%3,%4) returning ID',[login,passwordHash,email,nome]).then(res =>{
            result = res.rows
        })
        return result
    } catch (error) {
...
module.exports = {userRegister,getUserCredentials}
